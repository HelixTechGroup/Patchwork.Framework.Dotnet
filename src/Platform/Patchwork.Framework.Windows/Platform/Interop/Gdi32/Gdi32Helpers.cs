...
namespace Patchwork.Framework.Platform.Interop.Gdi32
{
    public static class Gdi32Helpers
    {
            ...
            DibBmiColorUsageFlag iUsage, out IntPtr ppvBits, IntPtr hSection, uint dwOffset)
        {
            ...
            {
                return Methods.CreateDIBSection(hdc, pbmi.GetDangerousHandle(), iUsage, out ppvBits, hSection,
                    dwOffset);
            }
        }
            ...
            DibBmiColorUsageFlag iUsage, out IntPtr ppvBits, IntPtr hSection, uint dwOffset)
        {
            ...
            {
                return Methods.CreateDIBSection(hdc, new IntPtr(bitmapInfoHeaderPtr), iUsage, out ppvBits, hSection,
                    dwOffset);
            }
        }
            ...
            DibBmiColorUsageFlag fuUsage)
        {
            using (var pbmi = BitmapInfo.NativeAlloc(ref bitmapInfo))
            {
                return Methods.CreateDIBitmap(hdc, ref lpbmih, fdwInit, lpbInit, pbmi.GetDangerousHandle(),
                    fuUsage);
            }
        }
            ...
            byte[] lpvBits, ref BitmapInfo bitmapInfo, DibBmiColorUsageFlag fuColorUse)
        {
            using (var pbmi = BitmapInfo.NativeAlloc(ref bitmapInfo))
            {
                return Methods.SetDIBitsToDevice(hdc, xDest, yDest,
