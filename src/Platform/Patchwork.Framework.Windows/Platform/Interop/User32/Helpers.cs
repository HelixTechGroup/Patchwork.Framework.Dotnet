...
namespace Patchwork.Framework.Platform.Interop.User32
{
    public static class User32Helpers
    {
            ...
            out WindowClassEx classInfo)
        {
            ...
            if (Methods.GetClassInfoEx(hInstance, lpClassName, out classExBlittable))
            {
                classInfo = new WindowClassEx
                {
                    ...
                    WindowProc = classExBlittable.WindowProc, /*Marshal.GetDelegateForFunctionPointer<WindowProc>(classExBlittable.WindowProc),*/
        ...
        {
            return Methods.PeekMessage(out lpMsg, hWnd, wMsgFilterMin, wMsgFilterMax, wRemoveMsg);
        }

        public static IntPtr GetNextWindow(IntPtr hwnd, GetWindowFlag cmd)
        {
            return Methods.GetNextWindow(hwnd, (uint) cmd);
        }

        public static IntPtr GetWindow(IntPtr hwnd, GetWindowFlag cmd)
        {
            return Methods.GetWindow(hwnd, (uint) cmd);
        }
        ...
        {
            return Methods.SystemParametersInfo((uint) uiAction, uiParam, pvParam, fWinIni);
        }
        ...
        {
            return Methods.MessageBox(hWnd, lpText, lpCaption, (uint) type);
        }
        ...
        public static uint SendInput(Input[] inputs)
        {
            ...
            try { return Methods.SendInput(len, gcHandle.AddrOfPinnedObject(), size); }
            finally { gcHandle.Free(); }
        }

        public static unsafe uint SendInput(ref Input input)
        {
            fixed (Input* ptr = &input) return Methods.SendInput(1, new IntPtr(ptr), Marshal.SizeOf<Input>());
        }
        ...
        public static unsafe bool GetTitleBarInfo(IntPtr hwnd, ref TitleBarInfo pti)
        {
            ...
            fixed (TitleBarInfo* ptr = &pti) return Methods.GetTitleBarInfo(hwnd, new IntPtr(ptr));
        }

        public static unsafe int MapWindowPoints(IntPtr hWndFrom, IntPtr hWndTo, ref Point point)
        {
            fixed (Point* ptr = &point) return Methods.MapWindowPoints(hWndFrom, hWndTo, new IntPtr(ptr), 1);
        }
        ...
        public static unsafe int MapWindowPoints(IntPtr hWndFrom, IntPtr hWndTo, ref Rectangle rect)
        {
            ...
            {
                var ptPtr = (Point*) ptr;
                return Methods.MapWindowPoints(hWndFrom, hWndTo, new IntPtr(ptPtr), 2);
            }
        }
        ...
        public static unsafe bool GetMonitorInfo(IntPtr hMonitor, out MonitorInfo lpmi)
        {
            ...
            return Methods.GetMonitorInfo(hMonitor, ref lpmi);
        }
            ...
            WindowExStyles dwExStyle)
        {
            ...
            var res = Methods.AdjustWindowRectEx(ref rc, dwStyle, hasMenu, dwExStyle);
